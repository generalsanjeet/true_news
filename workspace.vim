...
nnoremap <F6> :echo system('git symbolic-ref --short HEAD')
nnoremap <F5> :tabdo windo e
map <F2> :%s/\s\+$//e | autocmd TextChanged,TextChangedI syn off <buffer> silent write 
...
set winminwidth=0
set winwidth=1
exe 'vert 1resize ' . ((&columns * 100 + 62) / 125)
exe '2resize ' . ((&lines * 30 + 18) / 36)
exe 'vert 2resize ' . ((&columns * 24 + 62) / 125)
exe '3resize ' . ((&lines * 4 + 18) / 36)
exe 'vert 3resize ' . ((&columns * 24 + 62) / 125)
argglobal
terminal ++curwin ++cols=100 ++rows=35 
...
wincmd w
argglobal
terminal ++curwin ++cols=100 ++rows=30 
